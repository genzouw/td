...
- [ ] LLM プロバイダや従量課金 API のキー (`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `TAVILY_API_KEY` 等) を GitHub Secrets へ追加せず、`OPENAI_API_BASE_URL` などを課金可能なエンドポイントへ向ける CI 構成も追加していない
