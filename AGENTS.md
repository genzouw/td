...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述、および `OPENAI_API_BASE_URL` などを課金可能な endpoint へ向ける設定が追加・変更したファイルに含まれていない。
