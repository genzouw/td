...
> **運用に関する注記**: `free-policy` チェックは違反を検出すると **job が失敗** します (`enforce: true`)。導入直後 (2026-09-12) は誤検知を観察するため警告のみの運用でしたが、観察期間中に警告の発生がゼロだったため強制適用へ切り替えました。ただし本リポジトリのブランチ保護・ruleset は現時点でこのチェックを必須ステータスチェックに設定していないため、job の失敗自体は GitHub 上のマージを直接ブロックしません。違反が出ている PR は、原因を取り除くか `free-policy: allow <理由>` マーカーで除外してから、レビュー・マージすること。
